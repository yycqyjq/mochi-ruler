    ...
    分三层，逐层汇总：
      ...
      2. 五维分  该维各指标逐项分的加权平均（权重只表达相对重要性，由 _wavg()
         ...
         句法    越高越好（第三人称代词密度 / 代词起句率 / 句长中位）——
                 2026-09-13 新增。26 本真人长篇 vs 4 篇 AI 文本实测，
                 这三项是本工具里区分度最高的一组（代词密度与代词起句
                 两项 AUC = 1.00，两边完全不重叠）；而人工罗列的
...
标杆基准（BENCHMARKS）：
    7 本已完结中文长篇，每本抽前 40 章（≥1200 字），算各项中位，再取 7 本中位。
    键名用匿名代号 B1–B7，具体书目不入库。
"""
...
# `ttr`（词汇多样性）2026-09-13 起退出打分：实测 26 本真人长篇与 4 篇 AI
...
# 句法维（2026-09-13 新增）。三项都从「26 本真人长篇 vs 4 篇 AI 文本」的
...
def check(files):
    """达标验收：达标线取自 7 本标杆的中位数。输出逐章通过率与全书判定。"""
    rows = []
    for f in files:
...
BENCHMARKS = {
    # 键名为匿名代号，具体书目不入库（按项目要求：内容中不出现具体作品名）
...
def split_chapters(text):
    ...
    ≤24 字，且数字后不能是「年/月/日…」这类时间单位。实测在 26 本完结长篇
...
def _ttr(body, n=2):
    ...
    **2026-09-13 起退出打分，只算原始值备查。**
    区分度弱，不可当 AI / 真人的判据：同一实现下实测，26 本真人长篇与
    4 篇 AI 文本的逐章中位完全重叠（真人 11.6–82.8，AI 47.1–80.3），
    且其中一篇 AI 的 ttr 是全体最高。
