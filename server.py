...
SHOW = [
    ...
    # 淘汰理由（good() 的达标值可小于超标值，取反即可用）。加进去该维
    # 0.8812 → 0.9249、两群由重叠变不重叠。详见 qc_core 的 IMM_GOOD_BAD。
    'imm_cog', 'imm_perc', 'imm_soma', 'breath', 'surprise', 'touch_temp', 'act',
    # 句法（5）
...
DESC_TEXT = {
    ...
    'emo_type': '用到的不同情绪词种类数（笑/怒/惊/怕等，固定 18 字词表），按每百句归一。'
